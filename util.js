...
class SolarDate extends Calendar {
  ...
  static isValidDate(date) {
    ...
    if (date instanceof Date) {
      ...
      if (
        test_date.getFullYear() !== date.year ||
...
class LunarDate extends Calendar {
  ...
  static getSunLongitudeByJd(jd) {
    ...
    let DL =
      (1.9146 - 4817e-6 * T - 14e-6 * T2) * Math.sin(dr * M) +
      (0.019993 - 101e-6 * T) * Math.sin(dr * 2 * M) +
      29e-5 * Math.sin(dr * 3 * M);
    const theta = L0 + DL;
  ...
  static getSunLongitude(jd, timeZone) {
    return INT(
      (LunarDate.getSunLongitudeByJd(jd - 0.5 - timeZone / 24) / PI) * 12,
    );
  }
  ...
  getMonthName() {
    return (
      CAN[(this.year * 12 + this.month + 3) % 10] +
      " " +
      CHI[(this.month + 1) % 12] +
